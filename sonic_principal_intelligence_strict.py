#!/usr/bin/env python3
"""
SONiC Principal Intelligence Agent - Strict Guidelines Compliance
...
class SONiCPrincipalIntelligenceAgent:
    """SONiC Principal Intelligence Agent - Strict Guidelines Compliance"""
    ...
    def classify_file_strict(self, file_path: str) -> tuple:
        ...
        elif any(term in file_path_lower for term in ['redis', 'db', 'database']):
            layer = 'Database/Redis'
        elif any(term in file_path_lower for term in ['sai', 'asic', 'hardware']):
            layer = 'SAI'
    ...
    def extract_skills_strict(self):
        ...
        if category_counts.get('process', 0) > 0:
            skill = {
                ...
                'LEARNED_FROM': ['show_tech_analysis'],
                'CONFIDENCE': 'HIGH',
    ...
    def analyze_show_tech_strict(self, dump_file: str):
        """Perform analysis following strict guidelines"""
...
if __name__ == "__main__":
    ...
    agent = SONiCPrincipalIntelligenceAgent()
    result = agent.analyze_show_tech_strict(dump_file)
